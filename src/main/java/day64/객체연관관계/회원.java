...
public class 회원 {
    ...
    @Builder.Default  // 빌더 패턴 사용시 초기값이 대입되지 않는 멤버변수에 대해서는 초기값을 자동으로 대입한다.
    List<게시물> 작성한게시물 = new ArrayList<>();
    @Builder.Default
    @ToString.Exclude
    List<댓글> 작성한댓글 = new ArrayList<>();
    // 양방향 주의할 점 : 두 객체 서로 참조할 때는 순환참조(무한호출) 주의해야 한다.
    // 해결방안1 : ToString 메소드를 사용하지 않는다.
    // 해결방안2 : 양방향 멤버변수만 ToString 메서드를 사용하지 않는다. // @ToString.Exclude
